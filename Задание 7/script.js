...
let renderer = {
    ...
    render(snakePointArray, foodPoint, blockPoint=null) {
        ...
        this.cells[`x${foodPoint.x}_y${foodPoint.y}`].classList.add('food');
         ...
         for (var i = 0; i < settings.countBlocks; i++) {
            blockPoint = block.getBlockCoordinates(i);
            this.cells[`x${blockPoint.x}_y${blockPoint.y}`].classList.add('block');
    }
...
let settings = {
    ...
    speed: 2,
    winLength: 50,
    countBlocks: 10, //количество блоков на поле
...
let score = {
    ...
    init() {
        this.countEl = document.getElementById("score-field");
        this.drop();
    },
...
let block = {
    ...
    setBlockCoordinates(point, index) {
        this.mass[index].x = point.x;
        this.mass[index].y = point.y;
    },
    ...
    isBlockPoint(point, index) {
        return this.mass[index].x === point.x && this.mass[index].y === point.y;
    }
};
...
let game = {
    ...
    reset() {
        ...
        for (var i = 0; i < settings.countBlocks; i++) {
         this.block.setBlockCoordinates(this.getRandomCoordinates(), i);     
        }
    ...
    tickHandler() {
       ...
       for (var i = 0; i < settings.countBlocks; i++) {
        if (this.block.isBlockPoint(this.snake.getNextStepHeadPoint(),i)){
            this.finish();
        }
       } 
    ...
    getRandomCoordinates() {
        ...
        for (var i = 0; i < settings.countBlocks; i++) {
                    exclude.push(this.block.getBlockCoordinates(i))
                }
        while (true) {
